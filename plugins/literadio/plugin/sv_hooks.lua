...
function PLUGIN:PlayerCanRadio(player, text, listeners, eavesdroppers)
	local factionRadio = Clockwork.faction:FindByID(player:GetFaction()).radio;
	
	if (player:HasItemByID("handheld_radio") or factionRadio) then
...
function PLUGIN:PlayerAdjustRadioInfo(player, info)
	for k, v in ipairs( _player.GetAll() ) do
		if (v:HasInitialized() and (v:HasItemByID("handheld_radio") or Clockwork.faction:FindByID(v:GetFaction()).radio)) then
